...
const EVENTS = new Set(["pendakian-bersama", "ekspedisi", "private-trip"]);
...
export async function POST(request) {
  try {
    ...
    if (!EVENTS.has(eventSlug)) return NextResponse.json({ error: "Event tidak tersedia." }, { status: 400 });
    if (fullName.length < 2 || phone.length < 8 || emergencyContactName.length < 2 || emergencyContactPhone.length < 8) {
      return NextResponse.json({ error: "Lengkapi data peserta dan kontak darurat." }, { status: 400 });
    }
    ...
    const { data: existing, error: existingError } = await admin.from("event_registrations").select("id,status").eq("event_slug", eventSlug).eq("user_id", user.id).maybeSingle();
    ...
    const { data: created, error: insertError } = await admin.from("event_registrations").insert({
      ...
      notes: notes || null,
      status: "pending",
    }).select("id,event_slug,status,created_at").single();

    if (insertError) {
      if (insertError.code === "23505") return NextResponse.json({ error: "Kamu sudah terdaftar pada event ini." }, { status: 409 });
