...
export async function GET(request) {
  try {
    ...
    let query = supabase.from("products").select("id,name,slug,description,price,stock,image_url,is_active,category,subcategory,activities,created_at").eq("is_active", true).gte("created_at", "1970-01-01T00:00:00.000Z");
    ...
    const mapProduct = (row) => {
      ...
      return {
        id: row.id, code: String(row.slug || row.id).slice(0, 10).toUpperCase(), name: row.name, slug: row.slug,
        category: row.category || "Lainnya", subcategory: row.subcategory || "Lainnya", kind: row.subcategory || "Lainnya",
        activities: Array.isArray(row.activities) ? row.activities : [], price: Number(row.price || 0), oldPrice: null,
        badge: Number(row.stock || 0) <= 0 ? "OUT OF STOCK" : "AVAILABLE", image, imageUrl: image,
        blurb: row.description || "Perlengkapan outdoor pilihan Oxygen Gear.", description: row.description || "Perlengkapan outdoor pilihan Oxygen Gear.", stock: Number(row.stock || 0),
      };
    };
    if (slug) {
