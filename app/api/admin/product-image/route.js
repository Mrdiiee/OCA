...
export async function POST(request) {
  ...
  if (!buffer.length || buffer.length > MAX_SIZE) return NextResponse.json({ error: 'Ukuran foto maksimal 5 MB.' }, { status: 400 });
  const original = decodeURIComponent(request.headers.get('x-file-name') || 'product').replace(/[^a-zA-Z0-9._-]/g, '-').slice(0, 100);
  const ext = type === 'image/jpeg' ? 'jpg' : type.split('/')[1];
  const safeName = original.replace(/\.[^.]+$/, '') || 'product';
  const path = `products/${crypto.randomUUID()}-${safeName}.${ext}`;
  const { error } = await c.admin.storage.from('product-images').upload(path, buffer, { contentType: type, upsert: false, cacheControl: '31536000' });
  if (error) { console.error(error); return NextResponse.json({ error: 'Foto produk gagal diunggah.' }, { status: 500 }); }
  const { data } = c.admin.storage.from('product-images').getPublicUrl(path);
