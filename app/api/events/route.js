...
export async function GET() {
  ...
  const { data, error } = await supabase.from('events').select('*').eq('published', true).order('sort_order', { ascending: true }).order('event_date', { ascending: true }).order('created_at', { ascending: true });
