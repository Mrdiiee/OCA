...
export async function POST(request) {
  try {
    ...
    if (!checkoutKey || checkoutKey.length > 46) {
      return NextResponse.json({ error: "Checkout key tidak valid. Silakan coba lagi." }, { status: 400 });
    }
    ...
    const { data: existing, error: existingError } = await admin
      .from("orders")
      .select("id,order_number,midtrans_snap_token,status,total_amount")
      .eq("user_id", user.id)
      .eq("checkout_idempotency_key", checkoutKey)
      .maybeSingle();
    if (existingError) {
      return NextResponse.json({ error: "Checkout tidak dapat diverifikasi." }, { status: 500 });
    }
    if (existing?.midtrans_snap_token) {
    ...
    const { data: products, error: pe } = await admin
      ...
      .in("id", ids);
    if (pe) {
      return NextResponse.json({ error: "Produk gagal diverifikasi." }, { status: 500 });
    }
    ...
    const verified = requested.map((i) => {
    ...
    if (verified.some((i) => !i.is_active || i.quantity > i.stock)) {
      return NextResponse.json(
        { error: "Stok produk tidak mencukupi atau produk sudah tidak aktif. Keranjang telah berubah, silakan cek lagi." },
        { status: 409 }
      );
    }
    ...
    if (!order) {
      ...
      const { data: created, error: oe } = await admin
        .from("orders")
        .insert({
          user_id: user.id,
      ...
      if (oe) {
        const { data: race } = await admin
          ...
          .eq("checkout_idempotency_key", checkoutKey)
          .maybeSingle();
        if (race?.midtrans_snap_token) {
          return NextResponse.json({ token: race.midtrans_snap_token, orderId: race.order_number, reused: true });
        }
        if (!race) {
          return NextResponse.json({ error: "Pesanan gagal disiapkan. Silakan coba lagi." }, { status: 500 });
        }
        order = race;
      } else {
        order = created;
    ...
    if (Number(order.total_amount) !== total) {
      return NextResponse.json({ error: "Checkout ini sudah dipakai untuk pesanan dengan total berbeda." }, { status: 409 });
    }
    ...
    if (savedItemsError) {
      return NextResponse.json({ error: "Detail pesanan tidak dapat diverifikasi. Silakan coba lagi." }, { status: 500 });
    }

    if (savedItems?.length) {
      const sameItems =
        savedItems.length === verified.length &&
        verified.every((v) =>
          savedItems.some(
            (s) =>
              s.product_id === v.product_id &&
              Number(s.quantity) === v.quantity &&
              Number(s.unit_price) === v.unit_price
          )
        );
      if (!sameItems) {
        return NextResponse.json({ error: "Checkout ini sudah dipakai untuk keranjang yang berbeda." }, { status: 409 });
      }
    } else {
      ...
      if (ie) {
        const { data: concurrentItems } = await admin
          .from("order_items")
          .select("product_id,quantity,unit_price")
          .eq("order_id", order.id);
        const sameItems =
          concurrentItems?.length === verified.length &&
          verified.every((v) =>
            concurrentItems.some(
              (s) =>
                s.product_id === v.product_id &&
                Number(s.quantity) === v.quantity &&
                Number(s.unit_price) === v.unit_price
    ...
    if (orderWasCreated) {
      const { error: eventError } = await admin.from("order_tracking_events").insert({
        order_id: order.id,
        status: "pending_payment",
    ...
    const midtransRes = await fetch(getMidtransSnapUrl(), {
      ...
      body: JSON.stringify({
        ...
        customer_details: {
          ...
          phone: String(phone).trim(),
          billing_address: { address: String(address).trim() },
        },
        item_details: verified.map((i) => ({
          id: i.product_id,
          price: i.unit_price,
          quantity: i.quantity,
          name: i.product_name,
    ...
    if (!midtransRes.ok) {
      return NextResponse.json(
        {
          error:
            (Array.isArray(midtrans.error_messages) && midtrans.error_messages.join(", ")) ||
            "Gagal membuat transaksi Midtrans. Pesanan tetap tersimpan dan dapat dicoba kembali.",
        },
        { status: 500 }
      );
    }
    if (!midtrans.token) {
      return NextResponse.json({ error: "Midtrans tidak mengembalikan token pembayaran. Silakan coba lagi." }, { status: 502 });
    }

    const { error: updateError } = await admin
      .from("orders")
      .update({ midtrans_snap_token: midtrans.token })
      .eq("id", order.id)
      .is("midtrans_snap_token", null);
    if (updateError) {
      console.error("Gagal menyimpan Snap token:", updateError);
      return NextResponse.json(
        { error: "Transaksi pembayaran dibuat, tetapi token belum tersimpan. Silakan coba lagi dengan tombol pembayaran yang sama." },
        { status: 500 }
      );
    }
