    ...
    <main className="site-shell">
      ...
      {checkoutOpen && <div className="overlay" onClick={() => {setCheckoutOpen(false);setConfirmed(false)}}><section className="modal" onClick={(e) => e.stopPropagation()}>
        {!confirmed ? <>
          ...
          <p className="empty-text">Mode uji coba. Pembayaran asli belum terhubung ke payment gateway.</p>
          <form className="form" onSubmit={submitOrder}>
            ...
            <button type="submit" className="btn btn-primary">Konfirmasi pesanan (uji coba)</button>
          </form>
        </> : <div className="confirmation"><span className="drawer-title">Pesanan tercatat</span><p className="empty-text">Ini masih simulasi checkout. Setelah payment gateway aktif, pelanggan akan menerima instruksi pembayaran di sini.</p><button className="btn btn-ghost" onClick={() => {setCheckoutOpen(false);setConfirmed(false);setCart([])}}>Selesai</button></div>}
      </section></div>}
    </main>
  );
}
