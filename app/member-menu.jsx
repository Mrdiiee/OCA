...
function styleItem(item, index, items) {
  ...
  if (span) Object.assign(span.style, {
    display: "block",
    marginTop: "6px",
    color: "#777",
    fontSize: "11px",
...
function polishMobileMenu() {
  ...
  style.textContent = `
    ...
    .og-menu-trigger::after{content:"MENU"!important;}
    .og-menu-trigger{font-size:0!important;}
    .og-menu-trigger:hover{border-color:#e1261c!important;color:#e1261c!important;transform:translateY(-1px)!important;box-shadow:0 11px 30px rgba(225,38,28,.13)!important}
    .og-menu-trigger:focus-visible{outline:2px solid #e1261c!important;outline-offset:3px!important}
    .og-menu-icon{display:none!important}
    @media(max-width:980px){
      .shell .more-menu{position:fixed!important;top:72px!important;right:14px!important;left:auto!important;width:min(360px,calc(100vw - 28px))!important;max-height:calc(100vh - 90px)!important;overflow:auto!important;padding:12px!important;border:1px solid #e2e2df!important;border-top:3px solid #e1261c!important;border-radius:18px!important;background:rgba(255,255,255,.985)!important;box-shadow:0 24px 70px rgba(0,0,0,.18)!important;backdrop-filter:blur(18px)!important}
      .shell .more-menu .more-item{border-radius:12px!important;margin:3px 0!important;padding:14px!important}
      .shell .more-menu .more-item:hover{background:#f6f6f4!important;color:#e1261c!important}
      .shell .more-menu .more-section{margin:2px 0 8px!important;padding:14px!important;border:0!important;background:#f7f7f5!important;border-radius:12px!important}
  ...
  const onDocumentClick = (event) => {
    if (!menu.contains(event.target) && !trigger.contains(event.target)) {
      if (getComputedStyle(menu).display !== "none") trigger.click();
    }
  };
  const onKeyDown = (event) => {
    if (event.key === "Escape" && getComputedStyle(menu).display !== "none") {
      trigger.click();
      setTimeout(sync, 0);
      trigger.focus();
    }
